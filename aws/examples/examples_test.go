...
var cwd, _ = os.Getwd()
var examples = map[string]integration.ProgramTestOptions{
	"crawler": {
...
func ShouldRun(t *testing.T, test string) (bool, integration.ProgramTestOptions) {
	if region == "" {
		t.Skipf("Skipping test due to missing AWS_REGION environment variable")
		return false, integration.ProgramTestOptions{}
	}
	fmt.Printf("AWS Region: %v\n", region)
	return true, examples[test]
}

func TestCrawlerExample(t *testing.T) {
	t.Parallel()
	if run, test := ShouldRun(t, "crawler"); run {
...
func TestCountdownExample(t *testing.T) {
	t.Parallel()
	if run, test := ShouldRun(t, "countdown"); run {
		t.Run(test.Dir, func(t *testing.T) {
			integration.ProgramTest(t, test)
		})
	}
...
func TestContainersExample(t *testing.T) {
	t.Parallel()
	if run, test := ShouldRun(t, "containers"); run {
		t.Run(test.Dir, func(t *testing.T) {
			integration.ProgramTest(t, test)
		})
	}
...
func TestTodoExample(t *testing.T) {
	t.Parallel()
	if run, test := ShouldRun(t, "todo"); run {
		t.Run(test.Dir, func(t *testing.T) {
			integration.ProgramTest(t, test)
		})
	}
...
func TestTimersExample(t *testing.T) {
	t.Parallel()
	if run, test := ShouldRun(t, "timers"); run {
		t.Run(test.Dir, func(t *testing.T) {
			integration.ProgramTest(t, test)
		})
	}
...
func TestHttpEndpointExample(t *testing.T) {
	t.Parallel()
	if run, test := ShouldRun(t, "httpEndpoint"); run {
		t.Run(test.Dir, func(t *testing.T) {
			integration.ProgramTest(t, test)
		})
	}
