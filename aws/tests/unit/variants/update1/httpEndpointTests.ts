...
namespace updateProgramTests {
    ...
    export async function testInitialGet() {
        ...
        await supertest(address).get("stage/persistent1/").expect(200, { version: "1" });
    ...
    export async function testStaticGet() {
        const address = await deployment.url;
        await supertest(address).get("stage/persistent2/file2.txt").expect(200, "contents2\n");
        await supertest(address).get("stage/persistent2/file1.txt").expect(400);
    }
}
