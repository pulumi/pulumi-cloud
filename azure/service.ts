...
function computeImageNameForBuild(build: string | cloud.ContainerBuild) {
...
function computeImageFromBuild(
    ...
    const imageName = computeImageNameForBuild(build);
    ...
    const imageOptions =
        pulumi.all([registry.loginServer, dockerRegistry, imageName])
